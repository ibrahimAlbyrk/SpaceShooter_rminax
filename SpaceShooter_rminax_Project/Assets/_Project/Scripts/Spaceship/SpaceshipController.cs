...
namespace _Project.Scripts.Spaceship
{
    public sealed class SpaceshipController : NetworkBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (Physics.Raycast(ray, out hit, m_shooting.rocketSettings.LockOnDistance))
            {
                if (hit.transform != transform && Vector3.Distance(transform.position, hit.point) > 10f)
                {
                    ...
                    {
                        Cursor.SetCursor(m_camera.aimingCursor,
                            new Vector2(m_camera.aimingCursor.width * 0.5f, m_camera.aimingCursor.height * 0.5f),
                            CursorMode.Auto);
                    }
                }
                else
        ...
        {
            [Tooltip("The origin point(s) of rockets.")]
            public List<GameObject> RocketBarrels;
            ...
            [Tooltip("How long before the rocket disappears.")]
            public float RocketLifetime;

            [Tooltip("How fast does the rocket turn.")]
            public float RocketTurningSpeed;

            [Tooltip("Max distance of rocket lock-on")]
