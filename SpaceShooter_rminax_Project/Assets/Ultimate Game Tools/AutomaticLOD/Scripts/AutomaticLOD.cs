...
public class AutomaticLOD : MonoBehaviour
{
    ...
    {
        get { return s_userDefinedCamera; }
        set { s_userDefinedCamera = value; }
    }
    ...
    [HideInInspector] public EvalMode m_evalMode = EvalMode.ScreenCoverage;

    [HideInInspector] public bool m_bEnablePrefabUsage = false;

    [HideInInspector] public string m_strAssetPath = null;

    [HideInInspector] public float m_fMaxCameraDistance = 1000.0f;

    [HideInInspector] public int m_nColorEditorBarNewIndex = 0;

    [HideInInspector] public List<LODLevelData> m_listLODLevels;

    [HideInInspector] public AutomaticLOD m_LODObjectRoot;

    [HideInInspector] public List<AutomaticLOD> m_listDependentChildren = new List<AutomaticLOD>();

    public bool m_bExpandRelevanceSpheres = true;

    public RelevanceSphere[] m_aRelevanceSpheres = null;

    [SerializeField] private Simplifier m_meshSimplifier = null;

    [SerializeField] private bool m_bGenerateIncludeChildren = true;

    [SerializeField] private LevelsToGenerate m_levelsToGenerate = LevelsToGenerate._3;

    [SerializeField] private SwitchMode m_switchMode = SwitchMode.UnityLODGroup;

    [SerializeField] private bool m_bOverrideRootSettings = false;

    [SerializeField, HideInInspector] private bool m_bLODDataDirty = true;

    [SerializeField, HideInInspector]
    private AutomaticLOD m_LODObjectRootPersist = null; // Will persist even if you exclude it from the LOD tree

    [SerializeField, HideInInspector] private LODGroup m_LODGroup = null; // For SwitchMode == LODGroup
    ...
    void Update()
    {
        ...
        Camera renderCamera = s_userDefinedCamera != null ? s_userDefinedCamera :
            m_renderCamera != null ? m_renderCamera : Camera.main;

        if (IsDependent())
        {
            bool bCheck = m_LODObjectRootPersist != null
                ? m_LODObjectRootPersist.b_performCheck
                : m_LODObjectRoot.b_performCheck;

            if (bCheck)
            {
                // Dependent object

                renderCamera = s_userDefinedCamera != null
                    ? s_userDefinedCamera
                    :
                    (m_LODObjectRootPersist != null && m_LODObjectRootPersist.m_renderCamera != null)
                        ?
                        m_LODObjectRootPersist.m_renderCamera
                        : (m_LODObjectRoot != null && m_LODObjectRoot.m_renderCamera != null)
                            ? m_LODObjectRoot.m_renderCamera
                            : (m_renderCamera != null ? m_renderCamera : Camera.main);

                if (bCheck && renderCamera != null)
                {
                    int nLODLevel = m_LODObjectRootPersist != null
                        ? m_LODObjectRootPersist.GetLODLevelUsingCamera(renderCamera)
                        : m_LODObjectRoot.GetLODLevelUsingCamera(renderCamera);
  ...
  void OnWillRenderObject()
  {
    ...
    if (IsDependent())
    {
      ...
      bool bCheck =
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         m_LODObjectRootPersist != null ? m_LODObjectRootPersist.b_performCheck : m_LODObjectRoot.b_performCheck;

      if(bCheck)
      {
        int nLODLevel =
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    m_LODObjectRootPersist != null ? m_LODObjectRootPersist.GetLODLevelUsingCamera(Camera.current) : m_LODObjectRoot.GetLODLevelUsingCamera(Camera.current);
    ...
    void OnDrawGizmos()
    {
        ...
        RelevanceSphere[] aRelevanceSpheres =
            m_LODObjectRoot != null ? m_LODObjectRoot.m_aRelevanceSpheres : m_aRelevanceSpheres;
        ...
        {
            if (((UnityEditor.Selection.gameObjects[i] == this.gameObject) && m_LODObjectRoot == null) ||
                ((m_LODObjectRoot != null) && (UnityEditor.Selection.gameObjects[i] == m_LODObjectRoot.gameObject)))
            {
                bDrawVertices = true;
            }
        }
        ...
        {
            aSphereMatrices[nSphere] = Matrix4x4.TRS(aRelevanceSpheres[nSphere].m_v3Position,
                    Quaternion.Euler(aRelevanceSpheres[nSphere].m_v3Rotation), aRelevanceSpheres[nSphere].m_v3Scale)
                .inverse;
        }
        ...
        for (int nVertex = 0; nVertex < aVerticesWorld.Length; nVertex++)
        {
            for (int nSphere = 0; nSphere < aRelevanceSpheres.Length; nSphere++)
            {
                if (aRelevanceSpheres[nSphere].m_bExpanded)
                {
                    ...
                    {
                        Gizmos.DrawCube(aVerticesWorld[nVertex],
                            Vector3.one * UnityEditor.HandleUtility.GetHandleSize(aVerticesWorld[nVertex]) * 0.05f);
                        break;
                    }
                }
            }
    ...
    public static bool IsRootOrBelongsToLODTree(AutomaticLOD automaticLOD, AutomaticLOD root)
    {
        ...
        return (automaticLOD.m_LODObjectRoot == null) || (automaticLOD.m_LODObjectRoot == root) ||
               (automaticLOD == root) || (automaticLOD.m_LODObjectRoot == root.m_LODObjectRoot);
    }
    ...
    public float ComputeScreenCoverage(Camera camera)
    {
        ...
        for (int nObject = 0; nObject < m_listDependentChildren.Count; nObject++)
        {
            if (m_listDependentChildren[nObject] != null && m_listDependentChildren[nObject].m_originalMesh)
            {
                ...
                BuildCornerData(ref m_listDependentChildren[nObject]._corners,
                    m_listDependentChildren[nObject].m_originalMesh.bounds);

                for (int i = 0; i < m_listDependentChildren[nObject]._corners.Length; i++)
                {
                    Vector3 v3Viewport = camera.WorldToViewportPoint(m_listDependentChildren[nObject].transform
                        .TransformPoint(m_listDependentChildren[nObject]._corners[i]));
    ...
    public Vector3 ComputeWorldCenter()
    {
        ...
        for (int nObject = 0; nObject < m_listDependentChildren.Count; nObject++)
        {
            if (m_listDependentChildren[nObject] != null && m_listDependentChildren[nObject].m_originalMesh)
            {
                ...
                {
                    Vector3 v3World = i == 0
                        ? m_listDependentChildren[nObject].GetComponent<Renderer>().bounds.min
                        : m_listDependentChildren[nObject].GetComponent<Renderer>().bounds.max;
    ...
    public float ComputeViewSpaceBounds(Vector3 v3CameraPos, Vector3 v3CameraDir, Vector3 v3CameraUp, out Vector3 v3Min,
        out Vector3 v3Max, out Vector3 v3Center)
    {
    ...
    public void SetLODLevels(List<LODLevelData> listLODLevelData, EvalMode evalMode, float fMaxCameraDistance,
    ...
    public static void AddLODLevelRecursive(AutomaticLOD root, GameObject gameObject, int nLevel, bool bBefore,
        bool bCreateMesh, bool bRecurseIntoChildren)
    {
                ...
                if (bProcess)
                {
                    ...
                    if (bBefore)
                    {
                        if (nLevel == 0)
                        {
                            ...
                            {
                                automaticLOD.m_listLODLevels[0].m_fScreenCoverage =
                                    (automaticLOD.m_listLODLevels[0].m_fScreenCoverage +
                                     automaticLOD.m_listLODLevels[1].m_fScreenCoverage) / 2.0f;
                                automaticLOD.m_listLODLevels[0].m_fMaxCameraDistance =
                                    (automaticLOD.m_listLODLevels[0].m_fMaxCameraDistance +
                                     automaticLOD.m_listLODLevels[1].m_fMaxCameraDistance) / 2.0f;
                            }
                            else
                            {
                                ...
                                {
                                    automaticLOD.m_listLODLevels[0].m_fMaxCameraDistance =
                                        automaticLOD.m_fMaxCameraDistance * 0.5f;
                                }
                            }
                        }
                        else
                        {
                            data.m_fScreenCoverage = (automaticLOD.m_listLODLevels[nLevel - 1].m_fScreenCoverage +
                                                      automaticLOD.m_listLODLevels[nLevel].m_fScreenCoverage) / 2.0f;
                            data.m_fMaxCameraDistance = (automaticLOD.m_listLODLevels[nLevel - 1].m_fMaxCameraDistance +
                                                         automaticLOD.m_listLODLevels[nLevel].m_fMaxCameraDistance) /
                                                        2.0f;
                            data.m_fMeshVerticesAmount =
                                (automaticLOD.m_listLODLevels[nLevel - 1].m_fMeshVerticesAmount +
                                 automaticLOD.m_listLODLevels[nLevel].m_fMeshVerticesAmount) / 2.0f;
                            data.m_nColorEditorBarIndex = automaticLOD.m_nColorEditorBarNewIndex++;
                        }
                    ...
                    else
                    {
                        ...
                        if (nLevel == nLastLevel)
                        {
                            ...
                            data.m_fMaxCameraDistance = (automaticLOD.m_listLODLevels[nLastLevel].m_fMaxCameraDistance +
                                                         automaticLOD.m_fMaxCameraDistance) * 0.5f;
                            data.m_fMeshVerticesAmount =
                                automaticLOD.m_listLODLevels[nLastLevel].m_fMeshVerticesAmount * 0.5f;
                            data.m_nColorEditorBarIndex = automaticLOD.m_nColorEditorBarNewIndex++;
                        }
                        else
                        {
                        ...
                        if (bCreateMesh)
                        {
                            CheckForAdditionalLODSetup(root, automaticLOD, data,
                                nLevel == nLastLevel ? nLastLevel : nLevel + 1);
                        }
                    }
                }
            }
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                AddLODLevelRecursive(root, gameObject.transform.GetChild(nChild).gameObject, nLevel, bBefore,
                    bCreateMesh, bRecurseIntoChildren);
    ...
    public static void RemoveLODLevelRecursive(AutomaticLOD root, GameObject gameObject, int nLevel, bool bDeleteMesh,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (automaticLOD != null)
        {
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                ...
                {
                    if (nLevel < 0 || nLevel >= automaticLOD.m_listLODLevels.Count ||
                        automaticLOD.m_listLODLevels.Count == 1)
                    {
                        bProcess = false;
                    }
                }
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                RemoveLODLevelRecursive(root, gameObject.transform.GetChild(nChild).gameObject, nLevel, bDeleteMesh,
                    bRecurseIntoChildren);
            }
        }
    }
    ...
    private void ComputeLODDataRecursive(AutomaticLOD root, GameObject gameObject, bool bRecurseIntoChildren,
        Simplifier.ProgressDelegate progress = null)
    {
        ...
        if (automaticLOD != null)
        {
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                ...
                if (meshFilter != null)
                {
                    ...
                    IEnumerator enumerator = automaticLOD.m_meshSimplifier.ProgressiveMesh(gameObject,
                        automaticLOD.m_originalMesh, root.m_aRelevanceSpheres, automaticLOD.name, progress);
        ...
        if (bRecurseIntoChildren)
        {
            ...
            {
                ComputeLODDataRecursive(root, gameObject.transform.GetChild(nChild).gameObject, bRecurseIntoChildren,
                    progress);
    ...
    private static void SwitchToLODRecursive(AutomaticLOD root, GameObject gameObject, int nLODLevel,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
            ...
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                    ...
                    automaticLOD.m_nCurrentLOD != nLODLevel)
                {
                    if (automaticLOD.LODSwitchMode == SwitchMode.SwitchMesh)
                    {
                        ...
                        if (meshFilter != null)
                        {
                            Mesh newMesh = automaticLOD.m_listLODLevels[nLODLevel].m_bUsesOriginalMesh
                                ? automaticLOD.m_originalMesh
                                : automaticLOD.m_listLODLevels[nLODLevel].m_mesh;
                    ...
                    else if (automaticLOD.LODSwitchMode == SwitchMode.SwitchGameObject)
                    {
                        if (gameObject.GetComponent<Renderer>() != null)
                        {
                            gameObject.GetComponent<Renderer>().enabled =
                                automaticLOD.m_listLODLevels[nLODLevel].m_bUsesOriginalMesh;
                        }
                        ...
                        for (int i = 0; i < automaticLOD.m_listLODLevels.Count; i++)
                        {
                            if (automaticLOD.m_listLODLevels[i].m_gameObject != null)
                            {
                                automaticLOD.m_listLODLevels[i].m_gameObject.SetActive(
                                    automaticLOD.m_listLODLevels[nLODLevel].m_bUsesOriginalMesh == false &&
                                    i == nLODLevel && automaticLOD.gameObject.activeSelf);
                            }
                        }
                    }
    ...
    private static void ComputeLODMeshRecursive(AutomaticLOD root, GameObject gameObject, int nLevel,
        bool bRecurseIntoChildren, Simplifier.ProgressDelegate progress = null)
    {
        if (Simplifier.Cancelled)
        {
            return;
        ...
        if (automaticLOD != null)
        {
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                if (automaticLOD.m_meshSimplifier != null)
                {
                    ...
                    int nVertexCount =
                        Mathf.RoundToInt(fAmount * automaticLOD.m_meshSimplifier.GetOriginalMeshUniqueVertexCount());
                    ...
                    if (nVertexCount < automaticLOD.m_meshSimplifier.GetOriginalMeshUniqueVertexCount())
                    {
                        ...
                        IEnumerator enumerator = automaticLOD.m_meshSimplifier.ComputeMeshWithVertexCount(gameObject,
                            automaticLOD.m_listLODLevels[nLevel].m_mesh, nVertexCount,
                            automaticLOD.name + " LOD " + nLevel, progress);
                    ...
                    else
                    {
                        ...
                        if (automaticLOD.m_listLODLevels[nLevel].m_gameObject != null &&
                            automaticLOD.m_listLODLevels[nLevel].m_gameObject.GetComponent<Renderer>() != null)
                        {
                            automaticLOD.m_listLODLevels[nLevel].m_gameObject.GetComponent<Renderer>().enabled =
                                !automaticLOD.m_listLODLevels[nLevel].m_bUsesOriginalMesh;
                        }
                    }
                }
            }
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                ComputeLODMeshRecursive(root, gameObject.transform.GetChild(nChild).gameObject, nLevel,
                    bRecurseIntoChildren, progress);
    ...
    private static void RestoreOriginalMeshRecursive(AutomaticLOD root, GameObject gameObject, bool bDeleteLODData,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                RestoreOriginalMeshRecursive(root, gameObject.transform.GetChild(nChild).gameObject, bDeleteLODData,
                    bRecurseIntoChildren);
            }
        }
    }
    ...
    private static bool HasOriginalMeshActiveRecursive(AutomaticLOD root, GameObject gameObject,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                bHasOriginalMeshActive = bHasOriginalMeshActive || HasOriginalMeshActiveRecursive(root,
                    gameObject.transform.GetChild(nChild).gameObject, bRecurseIntoChildren);
            }
        }
    ...
    private static bool HasVertexDataRecursive(AutomaticLOD root, GameObject gameObject, int nLevel,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (automaticLOD != null)
        {
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                ...
                else if (automaticLOD.m_listLODLevels[nLevel].m_mesh &&
                         automaticLOD.m_listLODLevels[nLevel].m_mesh.vertexCount > 0)
                {
                    return true;
                }
            }
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                if (HasVertexDataRecursive(root, gameObject.transform.GetChild(nChild).gameObject, nLevel,
                        bRecurseIntoChildren))
                {
                    return true;
                }
    ...
    private static void GetOriginalVertexCountRecursive(AutomaticLOD root, GameObject gameObject, ref int nVertexCount,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                GetOriginalVertexCountRecursive(root, gameObject.transform.GetChild(nChild).gameObject,
                    ref nVertexCount, bRecurseIntoChildren);
            }
        }
    }
    ...
    private static void GetOriginalTriangleCountRecursive(AutomaticLOD root, GameObject gameObject,
        ref int nTriangleCount, bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                GetOriginalTriangleCountRecursive(root, gameObject.transform.GetChild(nChild).gameObject,
                    ref nTriangleCount, bRecurseIntoChildren);
            }
        }
    }
    ...
    private static void GetCurrentVertexCountRecursive(AutomaticLOD root, GameObject gameObject, ref int nVertexCount,
        bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                GetCurrentVertexCountRecursive(root, gameObject.transform.GetChild(nChild).gameObject, ref nVertexCount,
                    bRecurseIntoChildren);
            }
        }
    }
    ...
    private static void GetLODVertexCountRecursive(AutomaticLOD root, GameObject gameObject, int nLevel,
        ref int nVertexCount, bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                GetLODVertexCountRecursive(root, gameObject.transform.GetChild(nChild).gameObject, nLevel,
                    ref nVertexCount, bRecurseIntoChildren);
            }
        }
    }
    ...
    private static void GetLODTriangleCountRecursive(AutomaticLOD root, GameObject gameObject, int nLevel,
        ref int nTriangleCount, bool bRecurseIntoChildren)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                GetLODTriangleCountRecursive(root, gameObject.transform.GetChild(nChild).gameObject, nLevel,
                    ref nTriangleCount, bRecurseIntoChildren);
            }
        }
    }
    ...
    private static GameObject CreateBasicObjectCopy(GameObject gameObject, Mesh mesh, Transform parent)
    {
        ...
        for (int i = 0; i < aComponents.Length; i++)
        {
            ...
            if (c.GetType() == typeof(MeshRenderer) || c.GetType() == typeof(MeshFilter) ||
                c.GetType() == typeof(SkinnedMeshRenderer))
            {
    ...
    private static void CheckForAdditionalLODSetup(AutomaticLOD root, AutomaticLOD automaticLOD, LODLevelData levelData,
        int level)
    {
        if (automaticLOD.LODSwitchMode == SwitchMode.SwitchGameObject ||
            automaticLOD.LODSwitchMode == SwitchMode.UnityLODGroup)
        {
            ...
            levelData.m_gameObject = CreateBasicObjectCopy(automaticLOD.gameObject, levelData.m_mesh,
                automaticLOD.gameObject.transform);
    ...
    private static void SetupLODGroupRecursive(AutomaticLOD root, GameObject gameObject,
        ref List<List<Renderer>> renderers)
    {
        AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
            ...
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                if (automaticLOD.m_listLODLevels != null)
                {
                    ...
                    for (int i = 0; i < automaticLOD.m_listLODLevels.Count; ++i)
                    {
                        ...
                        Renderer renderer = data.m_bUsesOriginalMesh
                            ? gameObject.GetComponent<Renderer>()
                            : (data.m_gameObject != null ? data.m_gameObject.GetComponent<Renderer>() : null);
    ...
    static private Component CopyComponent(Component original, GameObject destination)
    {
        ...
        {
            if (!prop.CanWrite || !prop.CanWrite || prop.Name == "mesh" || prop.Name == "sharedMesh" ||
                prop.Name == "material" || prop.Name == "materials")
            {
                continue;
            }
    ...
    private static void DisablePrefabUsageRecursive(AutomaticLOD root, GameObject gameObject, bool bRecurseIntoChildren)
    {
            ...
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                if (automaticLOD.m_listLODLevels != null)
                {
                    foreach (LODLevelData data in automaticLOD.m_listLODLevels)
                    {
                        if (data.m_mesh)
                        {
                            if (UnityEditor.AssetDatabase.IsMainAsset(data.m_mesh) ||
                                UnityEditor.AssetDatabase.IsSubAsset(data.m_mesh))
                            {
        ...
        if (bRecurseIntoChildren)
        {
            for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
            {
                DisablePrefabUsageRecursive(root, gameObject.transform.GetChild(nChild).gameObject,
                    bRecurseIntoChildren);
            }
        }
    }
